...
class PointQuadTree:
    ...
    >>> tree = PointQuadTree(boundary=AxisAlignedBoundingBox(center=Point(8, 0), half_size=Point(8, 4)), node_capacity=2)
    >>> tree.boundary
    ...
    >>> tree.query_points_in_region(AxisAlignedBoundingBox(center=Point(0, 0), half_size=Point(8, 8)))
    [(7,1), (7,-1)]
    ...
    >>> tree.insert(obj2)
    True
    >>> tree.query_points_in_region(AxisAlignedBoundingBox(center=Point(0, 0), half_size=Point(2, 2)))
    [obj1:(0,0), obj2:(2,2)]
    ...
    def _calculate_subdivision_boundary(self, factor_x, factor_y):
        ...
        >>> tree = PointQuadTree(boundary=AxisAlignedBoundingBox(center=Point(1, 0), half_size=Point(2, 2)), node_capacity=1)
        >>> tree._calculate_subdivision_boundary(1, 1)
        AABB<center=(2.0,1.0), half_size=(1.0,1.0)>
        >>> tree._calculate_subdivision_boundary(-1, -1)
        AABB<center=(0.0,-1.0), half_size=(1.0,1.0)>
        """
        subdivision_half_size = Point(self.boundary.half_size.x / 2, self.boundary.half_size.y / 2)
        subdivision_center = self._calculate_subdivision_center(subdivision_half_size, factor_x, factor_y)
        return AxisAlignedBoundingBox(subdivision_center, subdivision_half_size)

    def _calculate_subdivision_center(self, subdivision_half_size, factor_x, factor_y):
        """
        ...
        """
        return Point(
            self.boundary.center.x + (factor_x * subdivision_half_size.x),
            self.boundary.center.y + (factor_y * subdivision_half_size.y))
